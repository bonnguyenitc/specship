...
- **Git:** work happens on the task's branch (`task/TASK-<ID>`, opened by `spec`); the checkpoint ends with a commit of `plan.md` + `task.md` (`TASK-<ID> plan: <event>`), staging exactly those files. See `../WORKFLOW.md` → Git flow.
